...
export default function TestCard({ title, description, selected, onClick }: TestCardProps) {
    return (
        <button
            onClick={onClick}
            className={`w-full h-40 text-left p-4 rounded-md transition shadow-[#c3ccd7] shadow-sm border border-[#c3ccd7] flex hover:bg-[#caddfe]/30 flex-col justify-between ${selected ? 'bg-[#caddfe]' : 'bg-white'}`}
        >
            ...
            <p className="text-gray-600 text-[10px]  sm:text-[14px] md:text-[1.1rem]">{description}</p>
        </button>
    );
}
